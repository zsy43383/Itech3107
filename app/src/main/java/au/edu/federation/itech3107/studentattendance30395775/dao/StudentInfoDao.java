...
public class StudentInfoDao {
    ...
    public ArrayList<StudentInfo> getStudentnumData(String num0) {
        ...
        if (num0 != null) {
            sql += "  where " + COL2 + "=?";
            cursor = this.st.rawQuery(sql, new String[]{num0});
        }else {
            cursor = this.st.rawQuery(sql, null);
        }
        ...
        while (cursor.moveToNext()) {
            ...
            @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(COL3));
            @SuppressLint("Range") String sex = cursor.getString(cursor.getColumnIndex(COL4));
            @SuppressLint("Range") String age = cursor.getString(cursor.getColumnIndex(COL5));
            @SuppressLint("Range") String pro = cursor.getString(cursor.getColumnIndex(COL6));
            @SuppressLint("Range") String mark = cursor.getString(cursor.getColumnIndex(COL7));
            //
            StudentInfo tem = new StudentInfo();
            //
            tem.setId(id);
    ...
    /**
     * 查询添加的学生信息  按 pro 课程进行查询
     *
     * @return ArrayList<StudentInfo>
     * @parma num0
     */
    public ArrayList<StudentInfo> getStudentByPro(String pro) {
        ArrayList<StudentInfo> adata = new ArrayList<StudentInfo>();
        //根据用户输入的学号查询单个学生信息
        String sql = "select * from  " + TB_NAME;
        //管理员 发送查寻指令
        ...
        if (pro != null) {
            sql += "  where " + COL6 + "=?";
            cursor = this.st.rawQuery(sql, new String[]{pro});
        }else {
            cursor = this.st.rawQuery(sql, null);
        }
        ...
        while (cursor.moveToNext()) {
            ...
            @SuppressLint("Range") String sex = cursor.getString(cursor.getColumnIndex(COL4));
            @SuppressLint("Range") String age = cursor.getString(cursor.getColumnIndex(COL5));
            @SuppressLint("Range") String pro1 = cursor.getString(cursor.getColumnIndex(COL6));
            @SuppressLint("Range") String mark = cursor.getString(cursor.getColumnIndex(COL7));
            //
            StudentInfo tem = new StudentInfo();
            //
